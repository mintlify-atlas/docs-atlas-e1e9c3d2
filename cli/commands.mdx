---
title: "Skillset CLI Command Reference"
sidebarTitle: "Commands"
description: "Full reference for every Skillset CLI command — flags, defaults, examples, and the outcomes each command can return."
---
...
<ParamField path="--agent" type="string">
  Which coding agent to install for. If omitted and a terminal is attached, the CLI presents an interactive picker. If omitted with no terminal, the agent is detected from the environment. Required under `--json`.
</ParamField>
...
<ParamField path="--json" type="boolean">
  Emit the result as JSON on stdout. Requires `--agent` and `--scope`.
</ParamField>
