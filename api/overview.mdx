---
title: "Skillset REST API Overview"
sidebarTitle: "Overview"
description: "Base URL, authentication methods, JSON request format, pagination envelope, common error codes, and rate-limiting rules for the Skillset REST API."
---
...
List endpoints that return many rows accept `page` and `page_size` query parameters and respond with a pagination envelope:
...
Default and maximum page sizes vary by endpoint; see each endpoint's own documentation for the exact limits.
...
Requests are rate-limited per client IP. When a load balancer or proxy sits in front of the Registry, set `TRUSTED_PROXY_IPS` in your environment to the proxy's address so the real client IP is read from `X-Forwarded-For` instead of the socket. Leaving it empty behind a proxy collapses all traffic into one shared rate-limit bucket.
