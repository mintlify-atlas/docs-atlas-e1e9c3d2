...
description: "Skillset is a self-hosted registry for Skills, MCP Servers, and Plugins — the resources a coding agent loads — hosted entirely on your own infrastructure."
---
...
A Skillset deployment runs as a single `docker compose up` command that starts one application container, a Postgres database, and a MinIO object-storage instance. Within that application, four components work together:
